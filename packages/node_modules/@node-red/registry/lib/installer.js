...
const npmRoot = path.dirname(npmPackageJson);
...
async function installModule(module,version,url) {
    ...
    activePromise = activePromise.then(async function() {
        ...
        } else if (validateNpmPackageName(module).validForOldPackages) {
            // A valid npm module name
            if (version) {
                installName += "@"+version;
            }
        ...
        if (!installAllAllowed) {
            ...
            if (isRegistryPackage) {
                // For an registry module, this will validate the version exists before trying to install it
                // It will throw an error if the version doesn't exist.
        ...
        return hooks.trigger("preInstall", triggerPayload).then((result) => {
            ...
            if (result !== false) {
                ...
                let args = [npmCli, 'install', ...extraArgs, '--', installName]
                log.trace(`${process.execPath} ${npmCli} ${JSON.stringify(args)}`);
                return exec.run(process.execPath, args, { cwd: installDir }, true)
            } else {
            ...
            if (err.hook) {
                ...
                if (err.hook === "postInstall") {
                    return exec.run(process.execPath, [npmCli, "remove", module],{ cwd: installDir }, false).finally(() => {
                        throw e;
                    })
                }
            } else {
...
async function getModuleVersionFromNPM(module, version) {
    ...
    return new Promise((resolve, reject) => {
        child_process.execFile(process.execPath, [npmCli, 'info', '--json', '--', installName], function(err,stdout,stderr) {
            try {
                if (!stdout) {
                    log.warn(log._("server.install.install-failed-not-found",{name:module}));
    ...
    activePromise = activePromise.then(() => {
        return new Promise((resolve,reject) => {
            ...
            return hooks.trigger("preUninstall", triggerPayload).then((result) => {
                ...
                if (result !== false) {
                    let extraArgs = triggerPayload.args || [];
                    let args = [npmCli, 'remove', ...extraArgs, '--', module]
                    log.trace(`${process.execPath} ${npmCli} ${JSON.stringify(args)}`);
                    return exec.run(process.execPath, args, { cwd: installDir }, true)
                } else {
...
async function checkPrereq() {
    ...
    if (settings.hasOwnProperty('editorTheme') &&
        ...
        return new Promise(resolve => {
            child_process.execFile(process.execPath, [npmCli, '-v'], function(err,stdout) {
                if (err) {
